...
export function AskForm({ groupId, dict }: { groupId: string; dict: Dictionary }) {
  ...
  async function ask(q: string) {
    ...
    if (isActionError(result)) {
      ...
      // shown as its own turn rather than swallowed — otherwise the question
      // simply vanished with no answer and no explanation.
      setTurns((prev) => [
        ...prev,
        { question: q, answer: result.error, sourceCounts: null },
      ]);
    } else {
      setTurns((prev) => [
        ...prev,
        { question: q, answer: result.answer, sourceCounts: result.sourceCounts },
      ]);
